...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile?.email) {
      toast.error('No se puede cambiar la contrasena sin un correo activo');
      return;
    }
    if (next.length < MIN_PASSWORD) {
      setConfirmError(`La contrasena debe tener al menos ${MIN_PASSWORD} caracteres`);
      return;
    }
    if (next !== confirm) {
      setConfirmError('La nueva contrasena y la confirmacion no coinciden');
      return;
    }
    ...
    try {
      ...
      if (updateError) {
        toast.error(`No se pudo actualizar la contrasena: ${updateError.message}`);
        return;
      }
    ...
    <Card className="bg-slate-900/40 border-slate-800">
      <CardHeader>
        ...
        <CardDescription className="text-slate-400">
          Usa al menos {MIN_PASSWORD} caracteres. Seguiras con sesion activa
          en este dispositivo despues de cambiarla.
        </CardDescription>
      </CardHeader>
