...
export function WhatsAppConfig() {
  ...
  async function handleSave() {
    if (!phoneNumberId.trim()) {
      toast.error('El ID del numero telefonico es obligatorio');
      return;
    }
    if (!config && (!accessToken.trim() || !tokenEdited)) {
      toast.error('El token de acceso es requerido para la configuracion inicial');
      return;
    }
    ...
    try {
      ...
      if (tokenEdited && accessToken !== MASKED_TOKEN && accessToken.trim()) {
        ...
        toast.error('Vuelve a ingresar el token de acceso para guardar cambios');
        setSaving(false);
        return;
      }
      ...
      if (payload.connected) {
        ...
        toast.success(
          payload.phone_info?.verified_name
            ...
            : 'Conexion con API correcta'
        );
      } else {
        setConnectionStatus('disconnected');
        setResetReason(payload.needs_reset ? 'token_corrupted' : payload.reason === 'meta_api_error' ? 'meta_api_error' : null);
        setStatusMessage(payload.message || '');
        toast.error(payload.message || 'La conexion con API fallo');
      }
    } catch (err) {
      console.error('Test connection error:', err);
      setConnectionStatus('disconnected');
      toast.error('La prueba de conexion fallo. Revisa la red e intenta de nuevo.');
    } finally {
      setTesting(false);
    }
  }
  ...
  async function handleReset() {
    if (!confirm('Esto eliminara la configuracion actual de WhatsApp para volver a capturarla. Continuar?')) {
      return;
    }
    ...
    try {
      ...
      toast.success('Configuracion eliminada. Ya puedes volver a capturar tus credenciales.');
      setConfig(null);
      setPhoneNumberId('');
      setWabaId('');
      setAccessToken('');
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_380px] mt-4">
      ...
      <div className="space-y-6">
        ...
        <Alert className="bg-slate-900 border-slate-700">
          ...
          <AlertDescription className="text-slate-400">
            {connectionStatus === 'connected'
              ? 'Tu API de WhatsApp Business esta conectada y lista para enviar y recibir mensajes.'
              : statusMessage ||
                'Configura tus credenciales de Meta para conectar tu cuenta de WhatsApp Business.'}
          </AlertDescription>
        </Alert>
        ...
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Ingresa tus credenciales de Meta WhatsApp Business API.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label className="text-slate-300">ID del numero telefonico</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-slate-300">ID de WhatsApp Business Account</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="text-slate-300">Token de verificacion del webhook</Label>
              <Input
              ...
              />
              <p className="text-xs text-slate-500">
                Debe coincidir con el token de verificacion que configures en Meta.
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400">
              Usa esta URL como callback del webhook en el panel de Meta.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
      ...
      <div>
        <Card className="bg-slate-900 border-slate-700 ring-0 ring-transparent">
          <CardHeader>
            <CardTitle className="text-white text-base">Instrucciones de configuracion</CardTitle>
            <CardDescription className="text-slate-400">
          ...
          <CardContent>
              ...
              <AccordionItem className="border-slate-700">
                ...
                <AccordionContent className="text-slate-400">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>Copia tu <strong className="text-slate-200">ID del numero telefonico</strong></li>
                    <li>Copia tu <strong className="text-slate-200">ID de WhatsApp Business Account</strong></li>
                    <li>Genera un <strong className="text-slate-200">token de acceso permanente</strong> desde Business Settings &gt; System Users</li>
                  </ol>
                </AccordionContent>
              </AccordionItem>
              ...
              <AccordionItem className="border-slate-700">
                ...
                <AccordionContent className="text-slate-400">
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    ...
                    <li>Haz clic en &quot;Edit&quot; en la seccion Webhook</li>
                    <li>Pega la <strong className="text-slate-200">URL de callback del webhook</strong> de arriba</li>
                    <li>Ingresa el mismo <strong className="text-slate-200">token de verificacion</strong> que configuraste aqui</li>
                    <li>Suscribete al campo webhook &quot;messages&quot;</li>
                  </ol>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
            ...
            <div className="mt-4 pt-4 border-t border-slate-700">
              ...
              >
                ...
                Documentacion de Meta WhatsApp API
              </a>
            </div>
          </CardContent>
        </Card>
