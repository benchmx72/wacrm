...
function MediaUnavailable({ label }: { label: string }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg bg-slate-700/40 px-3 py-2 text-xs text-slate-300">
      ...
      <span>{t("inbox.bubble.mediaUnavailable", { label })}</span>
    </div>
  );
}
  ...
  switch (message.content_type) {
    ...
    case "image":
      return (
        <div>
          {message.media_url ? (
            <MediaImage url={message.media_url} alt={t("inbox.bubble.sharedImage")} />
          ) : (
            <MediaUnavailable label={t("inbox.bubble.image")} />
          )}
          {message.content_text && (
      ...
      return (
        <div>
          {message.media_url ? (
            ...
            <MediaUnavailable label={t("inbox.bubble.video")} />
          )}
          {message.content_text && (
            <p className="mt-1 whitespace-pre-wrap break-words text-sm">
              {message.content_text}
    ...
    case "document":
      if (!message.media_url) {
        return <MediaUnavailable label={message.content_text || t("inbox.bubble.document")} />;
      }
      return (
        <a
        ...
        >
          ...
          <span className="truncate">
            {message.content_text || t("inbox.bubble.document")}
          </span>
        </a>
      );
    ...
    case "location":
      return (
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>{message.content_text || t("inbox.bubble.locationShared")}</span>
        </div>
      );
      ...
      return (
        <div className="flex flex-col gap-0.5">
          <span className="inline-flex items-center gap-1 text-[10px] font-medium uppercase tracking-wide text-slate-400">
            ...
            {t("inbox.bubble.buttonReply")}
          </span>
          <p className="whitespace-pre-wrap break-words text-sm">
            {message.content_text || t("inbox.bubble.interactiveReply")}
          </p>
        </div>
      );
    }
    ...
    default:
      return (
        <p className="whitespace-pre-wrap break-words text-sm">
          {message.content_text || t("inbox.bubble.unsupportedMessage")}
        </p>
      );
  }
}
