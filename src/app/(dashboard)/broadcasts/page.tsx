    ...
    <div className="space-y-6">
      ...
      {broadcasts.length === 0 ? (
        <div className="flex h-64 flex-col items-center justify-center rounded-xl border border-slate-800 bg-slate-900">
          ...
          <p className="text-sm font-medium text-white">{t('broadcasts.emptyTitle')}</p>
          <p className="mt-1 text-xs text-slate-400">
            {t('broadcasts.emptyHint')}
          </p>
          <Button
            onClick={() => router.push('/broadcasts/new')}
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-slate-800 hover:bg-transparent">
                <TableHead className="text-slate-400">{t('broadcasts.columns.name')}</TableHead>
                <TableHead className="hidden text-slate-400 md:table-cell">{t('broadcasts.columns.template')}</TableHead>
                <TableHead className="hidden text-right text-slate-400 sm:table-cell">
                  {t('broadcasts.columns.recipients')}
                </TableHead>
                <TableHead className="hidden text-slate-400 lg:table-cell">{t('broadcasts.columns.delivery')}</TableHead>
                <TableHead className="hidden text-slate-400 lg:table-cell">{t('broadcasts.columns.read')}</TableHead>
                <TableHead className="text-slate-400">{t('broadcasts.columns.status')}</TableHead>
                <TableHead className="hidden text-slate-400 sm:table-cell">{t('broadcasts.columns.date')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="hidden text-right text-slate-300 tabular-nums sm:table-cell">
                      {broadcast.total_recipients.toLocaleString(locale)}
                    </TableCell>
