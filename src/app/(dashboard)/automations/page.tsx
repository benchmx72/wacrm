...
const TEMPLATE_TEXT_KEYS = {
  welcome_message: {
    name: "automations.templates.welcome_message.name",
    description: "automations.templates.welcome_message.description",
  },
  out_of_office: {
    name: "automations.templates.out_of_office.name",
    description: "automations.templates.out_of_office.description",
  },
  lead_qualifier: {
    name: "automations.templates.lead_qualifier.name",
    description: "automations.templates.lead_qualifier.description",
  },
  follow_up_reminder: {
    name: "automations.templates.follow_up_reminder.name",
    description: "automations.templates.follow_up_reminder.description",
  },
} as const
...
export default function AutomationsPage() {
  ...
  async function toggleActive(a: Automation, next: boolean) {
    ...
    if (!res.ok) {
      ...
      toast.error(body?.error ?? t("automations.failedUpdate"))
      return
    }
  ...
  async function duplicate(a: Automation) {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("automations.failedDuplicate"))
      return
    }
  ...
  async function confirmDelete() {
    ...
    if (!res.ok) {
      const body = await res.json().catch(() => ({}))
      toast.error(body?.error ?? t("automations.failedDelete"))
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 text-sm font-semibold text-slate-300">{t("automations.templatesTitle")}</h2>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            {TEMPLATE_ORDER.map((slug) => {
              const Icon = TEMPLATE_ICON[slug]
              const textKeys = TEMPLATE_TEXT_KEYS[slug]
              return (
                <button
                  key={slug}
                ...
                >
                  ...
                  <div className="text-sm font-semibold text-white">{t(textKeys.name)}</div>
                  <p className="mt-1 text-xs text-slate-400">{t(textKeys.description)}</p>
                </button>
              )
            })}
          </div>
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-slate-800 bg-slate-900/40">
          ...
          <p className="mt-3 text-sm font-medium text-white">{t("automations.emptyTitle")}</p>
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {automation.is_active && (
              <span className="relative flex h-2 w-2" aria-label={t("common.activeStatus")}>
            ...
            <span className="tabular-nums">
              {t(
                automation.execution_count === 1
                  ? "automations.run"
                  : "automations.runs",
                { count: automation.execution_count.toLocaleString(locale) },
              )}
            </span>
            <span aria-hidden>·</span>
            <span>{t("automations.last", { time: formatRelative(automation.last_executed_at) })}</span>
          </div>
        </button>
        ...
        <div className="flex items-center gap-3">
          <Switch
            ...
            aria-label={automation.is_active ? t("common.deactivate") : t("common.activate")}
          />
